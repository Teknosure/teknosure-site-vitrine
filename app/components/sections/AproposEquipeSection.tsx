      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
          {equipe.map((member) => (
            <div
              key={member.name}
              className="group flex flex-col items-center rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[var(--primary-border)] hover:shadow-md"
            >
              {/* Avatar */}
              <div className="relative mb-4">
                <div className="flex h-20 w-20 items-center justify-center rounded-full bg-linear-to-br from-[var(--primary-dark)] to-cyan-400 text-xl font-bold text-white shadow-md ring-4 ring-white transition-all duration-300 group-hover:ring-[var(--primary-light)]">
                  {member.initials}
                </div>
                <Link
